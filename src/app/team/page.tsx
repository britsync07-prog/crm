  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-10 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-zinc-100 dark:border-white/5 pb-8">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-zinc-900 dark:text-zinc-50 uppercase italic flex items-center gap-3">
            <Users2 className="w-8 h-8 text-indigo-500" /> Human <span className="text-indigo-500 not-italic">Resources</span>
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-indigo-600 p-8 rounded-[40px] text-white space-y-6 shadow-2xl shadow-indigo-500/20 relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:rotate-12 transition-transform">
            <ShieldCheck className="w-24 h-24" />
          </div>
          <div className="relative z-10">
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-200 mb-2">Operation Status</p>
            <h3 className="text-3xl font-black italic">Fully Staffed</h3>
            <div className="flex items-center gap-4 mt-6">
              <div>
                <p className="text-2xl font-black">{employees.length}</p>
                <p className="text-[10px] font-bold uppercase text-indigo-200">Total Active</p>
              </div>
              <div className="w-px h-10 bg-white/20"></div>
              <div>
                <p className="text-2xl font-black">{employees.filter(e => e.attendances.length > 0).length}</p>
                <p className="text-[10px] font-bold uppercase text-indigo-200">On Duty</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {departments.map(dept => (
              <button key={dept} className="px-4 py-1.5 rounded-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:border-indigo-500 hover:text-indigo-500 transition-all">
                {dept}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {employees.map((emp) => (
            <div key={emp.id} className="bg-white dark:bg-zinc-950 p-8 rounded-[40px] border border-zinc-200 dark:border-white/10 shadow-sm relative group hover:border-indigo-500/30 transition-all">
              ...
              <div className="mt-8 pt-6 border-t border-zinc-100 dark:border-white/5 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm font-black text-indigo-600 mt-1">${emp.salary?.toLocaleString() || "0"}/mo</p>
                </div>
